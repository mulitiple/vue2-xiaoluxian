      ...
      <div class="main">
        ...
        <ul class="head">
          <li class="item check">
            <el-checkbox 
              :value="isChecked" 
              @change="allCheckFn">全选</el-checkbox>
          </li>
        ...
        <div v-if="shopList.length > 0">
          <ul class="haveorder"
            ...
            <li class="order-item">
              <el-checkbox
                :value="item.check"
                @change="checkItem(index)"></el-checkbox>
            </li>
            ...
            <li class="order-item delete" >
              <a href="javascript:;" >
                ...
                <span class="deletd-text" @click="deleteShopCar(item.id)">删除</span>
              </a>
            </li>
          </ul>
        </div>
...
<script>
import { getShopCarList,deleteShopCar } from '@/common/api/shopCar'
...
export default{
  ...
  methods:{
    ...mapActions(['allCheckFn']),
    ...mapMutations(['setShopCar','checkItem']),
    // 获取购物车数据
    getShopCarList(){
      getShopCarList().then(res => {
        this.setShopCar(res.data.list);
      })
    },
    // 删除购物车数据
    deleteShopCar(id){
